package wang.sunnly.micro.services.scannable.centre.auth.runner;
...
import wang.sunnly.micro.services.scannable.security.auth.core.utils.tools.RsaKeyHelper;
...
public class AuthServerRunner implements CommandLineRunner {
    ...
    //Redis中保存公钥秘钥的key值
    private final static String SUNNLY_REDIS_CLIENT_PUB_KEY = "SUNNLY:AUTH:CLIENT:PUB:KEY";
    private final static String SUNNLY_REDIS_CLIENT_PRI_KEY = "SUNNLY:AUTH:CLIENT:PRI:KEY";
    ...
    public void run(String... args) throws Exception {
        ...
        //首先从redis中获取公钥和秘钥封
        if(redisTemplate.hasKey(SUNNLY_REDIS_CLIENT_PUB_KEY)
                &&redisTemplate.hasKey(SUNNLY_REDIS_CLIENT_PRI_KEY)
            ...
            sshKeyProperties.setClientPriKey(RsaKeyHelper.toBytes(
                    redisTemplate.opsForValue().get(SUNNLY_REDIS_CLIENT_PRI_KEY)));
            sshKeyProperties.setClientPubKey(RsaKeyHelper.toBytes(
                    redisTemplate.opsForValue().get(SUNNLY_REDIS_CLIENT_PUB_KEY)));
            ...
            Map<String, byte[]> clientMap = RsaKeyHelper.generateKey(sshKeyProperties.getClientSecret());
            Map<String, byte[]> userMap = RsaKeyHelper.generateKey(sshKeyProperties.getUserSecret());
            sshKeyProperties.setClientPriKey(clientMap.get("priKey"));
            sshKeyProperties.setClientPubKey(clientMap.get("pubKey"));
            sshKeyProperties.setUserPriKey(userMap.get("priKey"));
            sshKeyProperties.setUserPubKey(userMap.get("pubKey"));

            //同时保存到redis中
            redisTemplate.opsForValue().set(SUNNLY_REDIS_CLIENT_PRI_KEY,
                    RsaKeyHelper.toHexString(clientMap.get("priKey")));
            redisTemplate.opsForValue().set(SUNNLY_REDIS_CLIENT_PUB_KEY,
                    RsaKeyHelper.toHexString(clientMap.get("pubKey")));
